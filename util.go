...
func LimitFileReader(filePath string) (*OmitString, error) {
...
func MakePublishing(resp Response, corId string) amqp091.Publishing {
...
func CompileError(code ErrorCode, err error, msg *OmitString, corId string) amqp091.Publishing {
	errMsg := ""
	if err != nil {
		errMsg = err.Error()
	}
...
func OKResp(resp ExecResult, corId string) amqp091.Publishing {
	rep := Response{
		ErrCode: OK,
		ErrMsg:  "success",
		Data:    resp,
